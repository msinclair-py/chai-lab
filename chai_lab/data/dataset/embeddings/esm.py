...
def esm_model(device="xpu:0"):
    ...
    if len(_esm_model) == 0:
        ...
        if device != torch.device("xpu:0"):
            ...
            model = torch.jit.load(local_esm_path, map_location="xpu:0").to(device)
        else:
            # skip loading on CPU.
    ...
    model.eval()
    yield model
    model.to("xpu")  # move model back to CPU when done
...
def _get_esm_contexts_for_sequences(
    prot_sequences: set[str], device='xpu'
...
@typecheck
def get_esm_embedding_context(chains: list[Chain], device='xpu') -> EmbeddingContext:
